...
pub fn build_routes(
    ...
    ping.or(version).boxed()
        .or(list_users_from_registry).boxed()
        .or(get_user_from_registry).boxed()
        .or(add_user_to_registry).boxed()
        .or(list_characters_for_user).boxed()
        .or(get_character_for_user).boxed()
        .or(claim_daily_for_user).boxed()
        .or(delete_user_from_registry).boxed()
        .or(draw_card_to_stage_for_user).boxed()
        .or(list_cards_from_compendium).boxed()
        .or(get_card_from_compendium).boxed()
        .or(put_card_to_compendium).boxed()
        .or(get_staged_card).boxed()
        .or(confirm_staged_card).boxed()
        .or(list_available_jobs).boxed()
        .or(list_jobs_for_user).boxed()
        .or(take_job_for_user).boxed()
        .or(get_job_for_user).boxed()
        .or(recall_job_for_user).boxed()
